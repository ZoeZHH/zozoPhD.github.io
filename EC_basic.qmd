...
<!-- -->
```
1)  Daytime fitting over moving windows partGLFitLRCWindows(), partGLFitLRCOneWindow()

2)  The light-response model families RectangularLRCFitter(), NonrectangularLRCFitter() / LogisticSigmoidLRCFitter()

3)  Bootstrap & Uncertainty .bootStrapLRCFit(), Prediction SDs in partGLInterpolateFluxes()

4)  From windows to HH, partGLInterpolateFluxes(), .partGPAssociateSpecialRows()

5)  Nighttime Reco override & covariances .computeRecoNight(),computeAggregatedCovariance() (helper)
