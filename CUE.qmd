...
NPP~ABG~ is determined allometric relationships with DBH

R~g~ = 25% \* NPP~ABG~

TBCA = f(litterfall)

NPP~BG~ = 0.5TBCA, Rg~BG~ = 0.25TBCA, Rm~BG~ = 0.25TBCA

Rm~sapwood~ = 0.07GPP OR Rm~stem&branch~ = 0.185GPP

Rm~leaves~ = relationship with nitrogen content of living tissue

NPP = NPP~ABG~ + NPP~BG~

GPP = R~g~ + Rm~leaves~ + Rm~stem&branch~ + Rg~BG~ + NPP~ABG~ + NPP~BG~
